...
## 실시간 감지 알림 (POST /api/alerts/live)

CCTV 실시간 화면에서 비정상이 감지되면 알림 내역에 바로 뜨는 경로입니다.

- **AI 서버가 호출합니다.** 인증 사용자가 없는 서버 간 호출이라 공개 경로이며, `videoSourceId`로 알림 받을 사람(카메라 소유자)을 정합니다.
- **중복 억제는 AI 쪽에서** 합니다(5연속 감지 + 재발동 쿨다운). 백엔드는 받은 대로 저장합니다.
- 알림 문구는 이 순서로 정해집니다:
  1. 요청에 `message`가 있으면 그대로
  2. 없으면 **사용자가 구독한 활성 케이스의 `out_msg`**
  3. 그것도 없으면 `"비정상 행동이 감지되었습니다."`
- **알림 내역에는 실시간 영상(CCTV/EDGE_GATEWAY)의 감지만 들어갑니다.** 업로드 영상 id로 호출하면 `400`으로 거절합니다.
- **업로드 영상 분석(배치)은 알림을 만들지 않습니다.** 영상을 저장할 때마다 알림이 쌓이지 않도록 실시간 경로와 분리했고, 배치 결과는 `GET /api/events`로 확인합니다.
